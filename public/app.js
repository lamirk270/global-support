(function(){
  ...
  ensureSid();
  let ended=(localStorage.getItem(ENDED_KEY)==='1'); let uid=(localStorage.getItem(UID_KEY)||'').trim();
  ...
  function showToast(kind, text){
    ...
    toast.style.animation = 'none'; toast.offsetHeight; // reset
  ...
  function unlockWithUID(val){
    uid=(val||'').trim(); if(!/^\d{8}$/.test(uid)){ invalidUIDFeedback(); showToast('failed','INVALID_UID'); return; }
    localStorage.setItem(UID_KEY, uid);
    uidModal.style.display='none';
    composer.style.display='flex';
    // set UID on server, then ensure session is active (continue)
    fetch('/api/status',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sessionId:sid,action:'uid',uid})}).then(()=>{
      ...
      localStorage.setItem(ENDED_KEY,'0'); setEnded(false); composer.style.display='flex'; showToast('connected','Connected'); loadHistory();
    });
    resetIdle();
  }
